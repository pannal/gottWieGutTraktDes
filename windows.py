    ...
    def showContextMenu(self):
        ...
        if select == -1:
            ...
            if removeMoviesFromWatchlist([movie]) == None:
                notification("Abgetrakt", __language__(1311).encode( "utf-8", "ignore" )) # Failed to remove from watch-list
            else:
                notification("Abgetrakt", __language__(1312).encode( "utf-8", "ignore" )) # Successfully removed from watch-list
                li.setProperty('Watchlist','false')
                movie['watchlist'] = False;
        elif actions[select] == 'watchlist':
            if addMoviesToWatchlist([movie]) == None:
                notification("Abgetrakt", __language__(1309).encode( "utf-8", "ignore" )) # Failed to added to watch-list
            else:
                notification("Abgetrakt", __language__(1310).encode( "utf-8", "ignore" )) # Successfully added to watch-list
                li.setProperty('Watchlist','true')
    ...
    def onAction(self, action):
        ...
        if action.getId() in (ACTION_PARENT_DIRECTORY, ACTION_PREVIOUS_MENU):
            ...
            if movie['idMovie'] == -1: # Error
                xbmcgui.Dialog().ok("Abgetrakt", movie['title'].encode( "utf-8", "ignore" ) + " " + __language__(1162).encode( "utf-8", "ignore" )) # "moviename" not found in your XBMC Library
            else:
                playMovieById(movie['idMovie'])
...
class TVShowsWindow(xbmcgui.WindowXML):
    ...
    def showContextMenu(self):
        ...
        if select == -1:
            ...
            xbmcgui.Dialog().ok("Abgetrakt", "comming soon")
        elif actions[select] == 'unwatchlist':
