...
def getNoBugging():
    __settings__ = xbmcaddon.Addon( "script.GottWieGutTraktDes" )
    return __settings__.getSetting( "noBugging" ) == "true"
...
def Debug(msg, force=False):
    ...
    if (debug == 'true' or force):
        try:
            ...
            print "GottWieGutTraktDes: " + msg.encode( "utf-8", "ignore" )
...
def checkSettings(daemon=False):
    if username == "":
        if daemon:
            notification("Abgetrakt", __language__(1106).encode( "utf-8", "ignore" )) # please enter your Username and Password in settings
        else:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1106).encode( "utf-8", "ignore" )) # please enter your Username and Password in settings
            __settings__.openSettings()
        return False
    elif __settings__.getSetting("password") == "":
        if daemon:
            notification("Abgetrakt", __language__(1107).encode( "utf-8", "ignore" )) # please enter your Password in settings
        else:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1107).encode( "utf-8", "ignore" )) # please enter your Password in settings
            __settings__.openSettings()
        return False
    ...
    if data == None: #Incorrect trakt login details
        if daemon:
            notification("Abgetrakt", __language__(1110).encode( "utf-8", "ignore" )) # please enter your Password in settings
        else:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1110).encode( "utf-8", "ignore" )) # please enter your Password in settings
            __settings__.openSettings()
        return False
...
def getTraktConnection():
    try:
        ...
        notification("Abgetrakt", __language__(1108).encode( "utf-8", "ignore" ) + ": timeout") # can't connect to trakt
        return None
    return conn
...
def traktJsonRequest(method, req, args={}, returnStatus=False, anon=False, conn=False, silent=False, passVersions=False):
    ...
    try:
        ...
        if not silent: notification("Abgetrakt", __language__(1108).encode( "utf-8", "ignore" )) # can't connect to trakt
        if returnStatus:
            data = {}
            data['status'] = 'failure'
    ...
    try:
        ...
        if returnStatus:
            ...
            return data
        if not silent: notification("Abgetrakt", __language__(1109).encode( "utf-8", "ignore" ) + ": Bad responce from trakt") # Error
        return None
    ...
    if 'status' in data:
        if data['status'] == 'failure':
            ...
            if returnStatus:
                return data;
            if not silent: notification("Abgetrakt", __language__(1109).encode( "utf-8", "ignore" ) + ": " + str(data['error'])) # Error
            return None
...
def setEpisodesSeenOnTrakt(tvdb_id, title, year, episodes):
    ...
    if data == None:
        ...
        for info in episodes:
            Debug("force-watching %s" % info)
            watchingEpisodeOnTrakt(None, title, None, info['season'], info['episode'], info['runtime'], 1)
            scrobbleEpisodeOnTrakt(tvdb_id, title, year, info['season'], info['episode'], info['runtime'], 100)
    return data
...
def rateMovieOnTrakt(imdbid, title, year, rating):
    ...
    if (rating == "unrate"):
        notification("Abgetrakt", __language__(1166).encode( "utf-8", "ignore" )) # Rating removed successfully
    else :
        notification("Abgetrakt", __language__(1167).encode( "utf-8", "ignore" )) # Rating submitted successfully
        ...
        try:
            if result['result'] == "OK":
                if xbmc.Player().isPlayingVideo():
                    return True
            notification("Abgetrakt", __language__(1302).encode( "utf-8", "ignore" )) # Unable to play movie
        except KeyError:
