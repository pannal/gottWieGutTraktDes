    ...
    if len(movie_collection) > 0:
        ...
        while last <= len(movie_collection):
            ...
            if data['status'] == 'success':
                ...
                if not daemon:
                    xbmcgui.Dialog().ok("Abgetrakt", __language__(1121).encode( "utf-8", "ignore" ), str(data['error'])) # Error uploading movie collection
        if not daemon:
                xbmcgui.Dialog().ok("Abgetrakt", str(inserted) + " " + __language__(1126).encode( "utf-8", "ignore" ), str(skipped) + " " + __language__(1138).encode( "utf-8", "ignore" )) # Movies updated on Trakt / Movies skipped
    else:
        if not daemon:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1122).encode( "utf-8", "ignore" )) # No new movies in XBMC library to update
...
def updateTVShowCollection(daemon=False):
    ...
    if count > 0:
        ...
        if error == None:
            ...
            if daemon:
                notification("Abgetrakt", __language__(1135).encode( "utf-8", "ignore" ) + str(error)) # Error uploading TVShow collection
            else:
                xbmcgui.Dialog().ok("Abgetrakt", __language__(1135).encode( "utf-8", "ignore" ), error) # Error uploading TVShow collection
    else:
        if not daemon:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1136).encode( "utf-8", "ignore" )) # No new episodes in XBMC library to update
...
def cleanMovieCollection(daemon=False):
    ...
    if len(to_unlibrary) > 0:
        ...
        if data['status'] == 'success':
            ...
            if not daemon:
                xbmcgui.Dialog().ok("Abgetrakt", str(data['message']))
        elif data['status'] == 'failure':
            Debug ("Error uploading movie collection: " + str(data['error']))
            if daemon:
                notification("Abgetrakt", __language__(1121).encode( "utf-8", "ignore" ), str(data['error'])) # Error uploading movie collection
            else:
                xbmcgui.Dialog().ok("Abgetrakt", __language__(1121).encode( "utf-8", "ignore" ), str(data['error'])) # Error uploading movie collection
    else:
        if not daemon:
			xbmcgui.Dialog().ok("Abgetrakt", __language__(1130).encode( "utf-8", "ignore" )) # No new movies in library to update
    if not daemon:
        progress.close()
...
def cleanTVShowCollection(daemon=False):
    ...
    if count > 0:
        ...
        if error == None:
            ...
            if daemon:
                notification("Abgetrakt", __language__(1135).encode( "utf-8", "ignore" ) + unicode(error)) # Error uploading TVShow collection
            else:
                xbmcgui.Dialog().ok("Abgetrakt", __language__(1135).encode( "utf-8", "ignore" ), error) # Error uploading TVShow collection
    else:
        if not daemon:
            xbmcgui.Dialog().ok("Abgetrakt", __language__(1142).encode( "utf-8", "ignore" )) # No episodes to remove from trakt
...
def syncSeenMovies(daemon=False):
    ...
    if len(movies_seen) > 0:
        ...
        if daemon:
            notification("Abgetrakt", str(len(movies_seen)) + " " + __language__(1129).encode( "utf-8", "ignore" )) # Movies updated on XBMC
        else:
